...
export class ConverterComponent {
  ...
  onChanges() {
    this.firstValue.valueChanges.pipe().subscribe((val: number) => {
      const { firstCurrency, secCurrency } = this.form.value;
      let res =
        val *
        (this.service.rates[firstCurrency] / this.service.rates[secCurrency]);
    ...
    this.secValue.valueChanges.subscribe((val) => {
      ...
      let res =
        val *
        (this.service.rates[secCurrency] / this.service.rates[firstCurrency]);
    ...
    this.firstCurrency.valueChanges.subscribe((newCurr) => {
      ...
      let res =
        firstValue *
        (this.service.rates[newCurr] / this.service.rates[secCurrency]);
      this.secValue.setValue(res, { emitEvent: false });
    });
    this.secCurrency.valueChanges.subscribe((newCurr: string) => {
      const { secValue, firstCurrency } = this.form.value;
      let res =
        secValue *
        (this.service.rates[newCurr] / this.service.rates[firstCurrency]);
      this.firstValue.setValue(res, { emitEvent: false });
    });
  }
}
